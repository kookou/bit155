...
public class BMICheck {
	//*******************************************************
	static HashMap<Integer, BMI> map = new HashMap<Integer, BMI>();  //value 값으로 객체(BMI) 를 가지고 있다.  //map 에값을 넣고 map 를 객체로 저장
	static int count = 1;
	...
	private void save() {
		...
		try{
			...
			ObjectOutputStream oos = new ObjectOutputStream(fos); //map 객체를 직렬화 시켜서 저장
			
			oos.writeObject(map);  //map 안에는 (key,value 1.new BMI, 2.new BMI, 3.new BMI) 
	...
	private void add() {
		BMI bmi = new BMI();
		map.put(count, bmi.input(bmi)); //4번 save
		count++;
	}
	...
	private void load() {
		...
		try{
			...
			map = (HashMap)oos.readObject(); //map 타입으로 다운 캐스팅
			
			Set<Integer> set = map.keySet();  //value 값 BMI 객체 
			System.out.println("번호\t키\t몸무게\t결과\t판정");
			for (Integer number : set) {
				double length = map.get(number).getLength(); //key(번호? 순서?), BMI객체 
				double weight = map.get(number).getWeight();
				double result = map.get(number).getResult();
				String you = map.get(number).getYou();
