...
class Buyer2{ //구매자
	...
	int bonuspoint;
	Product2 [] Cart;
	int index = 0;  //<어떤 배열방에 넣을지에 대한 위치정보
	...
	void Buy(Product2 product){
		...
		if(this.index >= 10){
			System.out.println("더이상 담을수 없습니다.");  //this.index 전역으로 뺀이요 모든 함수에서 사용가능
			return;
		}
		//장바구니 담기
		Cart[index++] = product; //후치증가 실행하고 증가 배열의 [0]번째 방부터
		//Cart[++index] = product; //전치증가 증가하고 실행 배열의 [1]번째 방에 넣는다 
